...
def adapt_data(data):
    ...
    data['data']['procuringEntity']['name'] = 'FOP_prozorro@bigmir.net'
    ...
    data['data']['items'][0]['classification']['scheme'] = adapt_scheme(data['data']['items'][0]['classification']['scheme'])
    return data
...
def convert_status(tender_status):
    status = {
        ...
        u'Блокування перед аукціоном': 'active.pre-qualification.stand-still'
    }
...
def adapt_scheme(data):
    value = {
        'CPV': u'ДК021'
    }
    return value[data]

def get_index_xpath(index):
    index_xpath = index+1
    return index_xpath
